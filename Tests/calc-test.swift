...
struct CalcTests {
    ...
    static func main() {
        ...
        expectError("10 usd cny", "No exchange rate for CNY.")
        let yuanRates = CurrencyRates(base: "USD", rates: ["CNY": 7.2], fetchedAt: clock.now)
        let yuanResult = CalcEngine.evaluate(
            "10 usd cny", now: clock.now, calendar: clock.calendar, rates: yuanRates)
        check(
            "10 usd cny with rates", expected: "true",
            got: "\(yuanResult?.payload == .value(display: "72.00 CNY", copyText: "72.00 CNY"))")
        expectDisplay("10 USD EUR", "9.20 EUR")
        expectDisplay("10usd eur", "9.20 EUR")
        expectDisplay("€20 GBP", "17.17 GBP")
        expectDisplay("2*50 usd eur", "92.00 EUR")
        expectDisplay("-5 usd eur", "-4.60 EUR")
        expectDisplay("0 usd eur", "0.00 EUR")
        expectDisplay("10 usd usd", "10.00 USD")
        expectDisplay("1 bitcoin usd", "60,000.00 USD")
        expectDisplay("10 usd eur", "9.20 EUR", region: "INR")
        expectCopy("100 dollars yen", "15700.00 JPY")
        expectExpression("10 usd eur", "10 USD")
        expectBadges("10 usd eur", source: "US Dollar", target: "Euro")
        expectErrorWithoutRates("10 usd eur", "Exchange rates unavailable — check your connection.")
        expectNil("usd eur")
        expectNil("10 usd nonsense")
        expectNil("10 usd cn")
        expectNil("10 usd eur jpy")
        expectNil("10 usd kg")
        expectNil("10 pounds usd")
        expectNil("10 usd pounds")
        expectNil("10 usdeur")
        expectNil("10 usd eur * 2")
